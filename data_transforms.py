...
class RandomCrop(object):
    ...
    def __call__(self, image, *labels):
        assert len(labels)==0 or image.size == labels[0].size, \
            "image and label doesn't have the same size {} / {}".format(
                image.size, labels[0].size)
        ...
        if left > 0 or right > 0 or top > 0 or bottom > 0:
            for i,l in enumerate(label_list):
                l = pad_image(
                    'constant', l, top, bottom, left, right, value=255)
                label_list[i] = l
            image = pad_image(
                'reflection', image, top, bottom, left, right)
        w, h = image.size

        # If size matches, return the image and labels
        if w == tw and h == th:
        ...
        if len(labels) > 0:
            for l in label_list:
                results.append(l.crop((x1, y1, x1 + tw, y1 + th)))
        # results.extend(args)
        return results
    ...
    def __call__(self, image, *labels):
        if random.random() < 0.5:
            if len(labels) > 0:
                ...
                for label in labels:
                    results.extend([label.transpose(Image.FLIP_LEFT_RIGHT)])
            else:
                results = [image]
        else:
            if len(labels) > 0:
...
class Normalize(object):
    ...
    Returns label as it is if present.
    """
    ...
    def __call__(self, image, *labels):
        ...
        #     return image,
        # else:
        return image, (*labels) # Returning a tuple (normalised_image, label1, label2, label3....)
...
class ToTensor(object):
    ...
    def __call__(self, pic, *labels):
        ...
        if len(labels) == 0:
            ...
            for i,l in enumerate(label_list):
                if l.mode == 'I':
                    label_list[i] = torch.FloatTensor(np.array(l, dtype=np.float32))
                else:
                    label_list[i] = torch.LongTensor(np.array(l, dtype=np.int))
